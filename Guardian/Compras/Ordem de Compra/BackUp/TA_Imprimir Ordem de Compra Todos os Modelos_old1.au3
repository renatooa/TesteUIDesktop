...
For $iContador = 1 To 10 Step +1
	...
	If ($iResultadoWinModelos) Then
		...
		If ($iResultadoWinReport) Then
			MsgBox($MB_ICONINFORMATION, "MODELO " & $iContador, "Modelo " & $iContador & " impresso com sucesso.", 1)
...
ConsoleWrite(WinActive("", $TEXTO_ORDEM_DE_COMPRA))

If (WinActive("", $TEXTO_ORDEM_DE_COMPRA)) Then	
