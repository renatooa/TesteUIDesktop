...
$sConfigBD = InputBox("Configuração Conexão Banco de Dados", _
		...
		"root | @senharootmysql123 | super100 | 192.168.0.7", "", 300, 150)
...
If (Not @error And UBound($aDadosConexaoBD) == 4) Then
	...
	$iTotalDeProdutos = UBound($aCodigoProdutos)
	...
	If ($iTotalDeProdutos > 0) Then
		...
		While ( true )
			...
			_ArrayShuffle($aCodigoProdutos) ; Embaralha os códigos
