...
Func _Assert($sCondition, $bExit = True, $iCode = 0x7FFFFFFF, $sLine = @ScriptLineNumber, Const $_iCurrentError = @error, Const $_iCurrentExtended = @extended)
	...
	If Not $bCondition Then
		MsgBox($MB_ICONERROR, "AutoIt Assert", "Assertion Failed (Line " & $sLine & "): " & @CRLF & @CRLF & $sCondition)
		If $bExit Then Exit $iCode
	EndIf

	If ($bCondition) Then
		MsgBox($MB_ICONINFORMATION, "AutoIt Assert", "Assertion Passed (Line " & $sLine & "): " & @CRLF & @CRLF & $sCondition)	
	EndIf
