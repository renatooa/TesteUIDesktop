...
#cs FUNCTIONS IN UDF ==============================================================================================================
    =
    =   _MySQLConnect($sUsername, $sPassword, $sDatabase, $sServer = "localhost", $sDriver = "{MySQL ODBC 5.1 Driver}", $iPort=3306)
    =   _Query($oConnectionObj, $sQuery)
    ...
    =   _CountTables($oConnectionObj)
    =   _GetColNames($oConnectionObj, $sTable)
    =   _GetTblNames($oConnectionObj)
    =   _GetColVals($oConnectionObj, $sTable, $sColumn)
    =   _GetColCount($oConnectionObj, $sTable)
    =   _GetColType($oConnectionObj, $sTable, $sColumn)
    =   _GetDBNames($conobj)
    =   _ChangeCon($oConnectionObj, $username = "", $password = "", $database = "", $driver = "", $server = "", $iPort = 0)
    =   _MySQLUpdate($sUsername, $sPassword, $sDatabase, $sHost, $sTabela, $aFieldsAndValues, $sWhereArgs)
    =
#ce ===============================================================================================================================
...
#cs
    ...
    Parameter(s):     $sUsername - The username to connect to the database with.
                      $sPassword - The password to connect to the database with. 
                      $sDatabase - Database to connect to.
                      $sHost - The server your database is on.
                      $sTabela - The table to the update.
    ...
    Author(s):        ronildo
#ce
Func _MySQLUpdate($sUsername, $sPassword, $sDatabase, $sHost, $sTabela, $aFieldsAndValues, $sWhereArgs)
    ...
    $oResultQuery = _Query($oMySqlConn, $sUpdate)   
